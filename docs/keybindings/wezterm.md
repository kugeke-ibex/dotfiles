...
`Cmd+Shift+A` で `agent_mode`（one_shot）に入り、次の 1 キーで各エージェント（または herdr）を
新規タブで起動する。実装は `modules/agents.lua`（`claude` / `codex` / `gemini` の生コマンドを送る）。
...
`agent_mode` は one_shot なので、未割り当てキーを押すと自動的に抜ける。git ブランチは
starship プロンプトに常時表示される（旧: タブタイトルへの表示は herdr 移行に伴い廃止）。
