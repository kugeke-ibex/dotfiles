...
| キー                      | 動作                                                  |
| ------------------------- | ----------------------------------------------------- |
| `s{char}{char}`           | 画面内を 2 文字で検索しラベルジャンプ                 |
| `S`                       | Treesitter ノード単位でジャンプ選択                   |
| `f` / `F` / `t` / `T`     | 行内ジャンプ（flash 強化版。`;` / `,` で次/前へ継続） |
| operator + `s`（例 `ds`） | オペレータ待ち中も flash で範囲指定可能               |
| `r`（オペレータ中）       | Remote（離れた位置を対象に操作）                      |
| `R`（ビジュアル）         | Treesitter Search で選択拡張                          |

> ビジュアルの `S` は flash（Treesitter）。囲み操作は `gs` / `gS`（[編集支援プラグイン](#編集支援プラグイン追加分)参照）。

### Git 操作（snacks picker / gitsigns / git.nvim）

| キー                          | 動作                                         |
| ----------------------------- | -------------------------------------------- |
| `<leader>gs`                  | Git status（変更ファイル picker）            |
| `<leader>gd` / `<leader>gD`   | Git diff（hunks / origin との差分）          |
| `<leader>gS`                  | Git stash 一覧                               |
| `<leader>ge`                  | Git Explorer（neo-tree）                     |
| `<leader>gi` / `<leader>gI`   | GitHub issues（open / all）                  |
| `<leader>gp` / `<leader>gP`   | GitHub Pull Request（open / all）            |
| `<leader>gtb` / `<leader>gtd` | git blame / diff（git.nvim）                 |
| `]h` / `[h`                   | 次 / 前の hunk（gitsigns、バッファローカル） |
| `<leader>uG`                  | Git signs 表示トグル                         |

> hunk のステージ/リセット等（`<leader>gh` プレフィックス）は gitsigns が attach したバッファでのみ有効。一覧は `<leader>gh` を押して which-key 参照。
...
| --------------------------- | ------------------------- |
| `]t` / `[t`                 | 次 / 前の TODO コメントへ |
| `<leader>st`                | TODO 一覧（picker）       |
| `<leader>sT`                | TODO/FIX/FIXME 一覧       |
| `<leader>xt` / `<leader>xT` | TODO を Trouble で開く    |

### スクラッチ & セッション（snacks / persistence）

| キー                        | 動作                                                                 |
| --------------------------- | -------------------------------------------------------------------- |
| `<leader>.`                 | スクラッチバッファをトグル（cwd × ft 別・自動永続。試し書き/メモ用） |
| `<leader>S`                 | スクラッチを一覧から選択                                             |
| `<leader>qs` / `<leader>ql` | セッション復元（このディレクトリ / 直近）                            |
| `<leader>qS` / `<leader>qd` | セッション選択 / 現在のセッションを保存しない                        |

### その他のよく使う操作

| キー                        | 動作                                                       |
| --------------------------- | ---------------------------------------------------------- |
| `<C-Space>`                 | Treesitter インクリメンタル選択（`<BS>` で縮小）           |
| `<C-a>` / `<C-x>`           | インクリメント / デクリメント（dial.nvim。`+`/`-` も同じ） |
| `gx`                        | カーソル下の URL / ファイルを OS で開く                    |
| `<leader>su`                | Undotree（編集履歴ツリー）                                 |
| `<leader>fp` / `<leader>fc` | プロジェクト / 設定ファイルを開く                          |
| `<leader>sn`                | noice（メッセージ履歴など、サブメニュー）                  |

> その他の picker は `<leader>s` / `<leader>f` を押して which-key 一覧で探せる。ノート管理（nb）は `<leader>n`（[nb.lua](../../config/nvim/lua/plugins/nb.lua) のヘッダ参照）。
