...
### 0. Caps Lock ↔ 左 Control（入れ替え）

`simple_modifications` で物理キーを入れ替える（右 Control はそのまま）。
...
Caps Lock 位置で vim 用の `Ctrl+h/j/k/l`（ルール 5）が使える。Caps Lock 相当の入力は旧左 Control 位置から。
...
ルール 0 の入れ替え後は、**Caps Lock 位置 + h/j/k/l** が矢印になる（送信上は `左 Ctrl` 修飾）。

vim 慣れの人がブラウザや Slack 等の非 vim アプリでもカーソル移動できるようにするためのルール。

### 6. Hotkey Window トグル (グローバル)

iTerm2 の Hotkey Window 相当。通常のターミナルウィンドウとは別に、ショートカットで表示/非表示を切り替える。

| キー         | 動作                                                                          |
| ------------ | ----------------------------------------------------------------------------- |
| `Ctrl+Opt+W` | WezTerm **hotkey ウィンドウ**の表示/非表示トグル（iTerm2 Hotkey Window 相当） |
| `Ctrl+Opt+G` | Ghostty **ウィンドウ**の表示/非表示トグル（同上）                             |

`⌃⌥` = Control + Option（Caps Lock ↔ 左 Control 入れ替え後は **Caps Lock 位置 + Option + W/G** でも可）。

- **WezTerm / Ghostty**: Karabiner → `karabiner-toggle-*` → `toggle-*-hotkey`。
- **推奨**: **Karabiner-Elements** にアクセシビリティ（ウィンドウの表示/非表示トグル用）。
- 反映: `cp` で `karabiner.json` → Karabiner 再起動、`Cmd+Shift+R` で Ghostty 設定リロード。

## トラブルシューティング

- **トグルできない**: **Karabiner-Elements** のアクセシビリティを確認。ログの `AX action=show|hide` を確認。
- **2回目で閉じない**: hotkey ウィンドウにフォーカスがある状態で再押下。
- **Caps Lock が効かない / 二重に効く**: システム設定 → キーボード → 「Caps Lock キーを…」を **なし**（または Karabiner と競合しない設定）にする
