...
| キー         | 動作 |
| ------------ | ---- |
| `Ctrl+Opt+W` | WezTerm **hotkey** ウィンドウをトグル（`~/.local/bin/toggle-wezterm-hotkey`） |
| `Ctrl+Opt+G` | Ghostty **Quick Terminal** をトグル（`config/ghostty/config` の `global:` キーバインド） |

`⌃⌥` = Control + Option。

- **WezTerm**: workspace `hotkey` のドロップダウンウィンドウ。`darwin-rebuild switch` で `toggle-wezterm-hotkey` が `~/.local/bin` に入る。
- **Ghostty**: アプリが起動している必要あり。初回は **システム設定 → プライバシーとセキュリティ → アクセシビリティ** で Ghostty を許可する（`global:` キーバインド用）。
- 通常の WezTerm / Ghostty ウィンドウは Dock や `Cmd+N` から従来どおり開ける。
