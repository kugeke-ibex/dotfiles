...
## herdr 内キーバインド（prefix = `Ctrl+B`）

tmux と同じく **prefix キーを押してから** 次のキーを押す（例: `Ctrl+B` → `c` で新規タブ）。
prefix は herdr 既定の `Ctrl+B` を使う（[config/herdr/config.toml](../../config/herdr/config.toml)）。
以下の表の `prefix` は `Ctrl+B` を指す。

> **なぜ `Ctrl+Space` にしないか**: macOS の「前の入力ソースを選択」(`AppleSymbolicHotKeys` 60、
> 既定で有効) が `Ctrl+Space` を占有しており、ターミナルより先にシステムが奪うため prefix に使えない
> （日本語 IME 切替とも競合）。`Ctrl+A` も候補だが shell の行頭移動を奪うため、無難な `Ctrl+B` を採用。
...
- 現状は prefix を既定の `ctrl+b` にしているだけ。以下は変更しうる主なキー（参考、既定値ベース）:

```toml
# prefix とキーバインドのカスタマイズ
[keys]
prefix = "ctrl+b"              # 既定のまま（ctrl+space は macOS 入力ソース切替と衝突するため不可）
new_tab = "prefix+c"
