...
`Cmd+Shift+A` を prefix にしたシーケンス。次の 1 キーで各エージェント（または herdr）を
現ペインで起動する（`claude` / `codex` / `gemini` の生コマンドを送るだけ）。
...
キーを使わず直接 `claude` / `codex` / `gemini` と打っても同じ。「1 タブ = 1 エージェント」で
手軽に回したいとき用で、本格的な並列管理・状態表示は `Cmd+Shift+A h` で起動する
[herdr](herdr.md) に委ねる。
